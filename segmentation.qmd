...
Uma empresa do varejo deseja aumentar suas vendas e otimizar campanhas de marketing, isto é, enviar campanhas específicas para públicos específicos pensando em um maior retorno sobre o investimento (ROI). Para isso, ela conta com uma base de dados com inúmeras informações sobre clientes contendo apsectos demográficos, padrões de compra e resultados de campanhas de marketing passadas. 
...
- A coleta de dados se deu aleatoreamente sem viéses de seleção.
...
A base original continha alguns dados ausentes, outliers (valores discrepantes) e colunas categóricas que precisaram ser codificadas (rótulos numéricos). Em resumo, obteve-se os seguintes insights da análise exploratória:

* Os clientes possuem entre 24-84 anos, com média em torno de 55 anos.
* Cerca de 89% dos clientes possuem ensino superior.
* A base conta com estados civis de: solteiro, casado, divorciado e viúvo. Aproximadamente 35% da base vive sem um companheiro.
* Cerca de 28% da base não tem filhos, enquanto 72% possui um ou mais filhos.
* Tem-se várias faixas de renda entre os clientes, com mínimo de 1.730 USD, máximo de 154.000 USD e média de 52.200 USD anuais (desconsiderando outliers).
...
* `NumWebVisitsMonth`: não necessariamente as visitas convertem em compra, logo foi decidido pelo negócio a exlusão dessa coluna na fase de modelagem. 
* `Complain`: 99% dos clientes não reclamaram, logo, essa coluna não contribuiria muito para a modelagem.
* `Response`: se refere à compra, mas apenas na última campanha enviada, pouco representativa para a modelagem.

Colunas modificas/novas colunas:

* `Year_Birth` foi substituída por `Age` (cálculo de datas)
* `Education` foi substituída por `Education_Level` que divide os clientes quanto a: *(i)* Básico, *(ii)* Graduação e *(iii)* Pós-Graduação.
...
* Colunas do tipo `AcceptedCmp...` foi derivada em  `AcceptedCmps` (número de campanhas aceitas)
* Nova coluna: `TotalPurchases` (Total de compras)
...
Nessa etapa foram calculadas correlações entre as variávies, colunas numéricas foram normalizadas em função de possuírem diferentes escalas. Já as colunas categórias, estas foram codificadas da seguinte forma:
...
A etapa de modelagem se deu via algoritmo K-means, um algoritmo de aprendizado não-supervisionado destinado a tarefas de agrupamento (oy clusterização). Antes de entrar na modelagem observou-se pelas as colunas categóricas e com base nas rendas anuais (coluna numérica), um número de grupos sugestivo para a modelagem:
...
1. É informado um número de clusters inicial, bem como o centro de cada cluster;
2. A distância de cada ponto versus o centro de cada cluster é calculada. Atribui-se a cada observação o cluster cuja distância em relação ao centro do cluster é menor.
...
Segmentando um pouco por fatores como aspectos demográficos, recência/frequência/valor
...
* Cluster A: em torno de 46 anos
* Clusters B, C, D: em torno de 60 anos
...
* Cluster A: Gasta em frutas, carnes, peixe, doces e, de certa frma, acaba comprando produtos premium (talvez em ofertas)
* Cluster B: gasta com vinhos e carnes
* Cluster C: gasta com vinhos e produtos premium
* Cluster D: gasta com vinhos, carnes, peixes e doces.
...
* Cluster C compra online e na loja, mas o volume de compras na loja é maior
...
* **Cluster A (baixa renda, poucas compras):** cliente com média 46 de anos, de baixa renda, possui ensino superior, entre 0-1 filhos e que valoriza cada centavo gasto. Suas compras são guiadas por promoções e descontos, e ela não é fiel a marcas específicas. Sua resposta a campanhas de marketing é baixa, preferindo informações de fontes confiáveis e próximas, como amigos e redes sociais. Esse tipo de cliente busca sempre o melhor custo-benefício para atender às suas necessidades e de sua família. Apesar disso, ele gasta em todas as categorias (carnes, frutas, doces, vinhos e produtos premium - este último em ofertas).

* **Cluster B (média renda, guiado por promoções e aposentado):**  cliente com média de 60 anos, possui ensino superior, é muito provavelmente aposentado e tem dois filhos adultos. Suas compras são guiadas por promoções e descontos, não sendo fiel a marcas específicas. Sua resposta a campanhas de marketing é baixa e esse tipo de cliente está sempre em busca de custo-benefício para atender às suas necessidades. Seu objetivo é economizar para garantir um futuro financeiro estável e aproveitar a aposentadoria sem preocupações. Costuma gastar em vinhos e carnes.

* **Cluster C (renda média-alta, cliente fiel, guiado pelo mix de preço e valor, aposentado):** cliente com média de 60 anos com renda média-alta, pós-graduado, casado e normalmente possui um único filho. Valoriza produtos premium e é um cliente fiel (grupo há mais tempo como cliente). Esse tipo de cliente compra tanto online quanto em lojas físicas e responde relativamente bem a campanhas de marketing. Além disso, pode-se dizer que é sensível a descontos e busca sempre o melhor valor em vinhos e produtos premium. Suas compras são guiadas pela qualidade e pelo desejo de economizar sem comprometer a excelência dos produtos. Ele busca desfrutar de um estilo de vida sofisticado e tem como objetivo continuar explorando novas experiências gastronômicas.

* **Cluster D (alta renda, guiado por marca e qualidade, aposentado):** cliente com média de 60 ano,  alta renda, com graduação, casado(a) e sem filhos. Se trata de um público de clientes fieis e que responde muito bem a campanhas de marketing. Compra tanto online quanto em lojas físicas e não é sensível a descontos, preferindo produtos de alta qualidade e exclusividade. Gasta consideravelmente em carnes, vinhos e peixes, sendo o tipo de cliente mais rentável. Suas compras são guiadas pela busca por qualidade e experiências premium. Além disso, deseja continuar explorando novas experiências sofisticadas e manter um estilo de vida confortável e farto.
...
* Cluster D: foco em produtos de alta qualdidade e que promovam um ótima experiência ao consumidor. Campanhas de marketing mais voltadas ao branding é interessante. Não oferecer muitas promoções. Esse cliente está disposto a pagar mais pela qualidade.